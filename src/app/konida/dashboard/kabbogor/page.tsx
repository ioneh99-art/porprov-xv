'use client'
// src/app/konida/dashboard/kabbandung/page.tsx — FINAL
// Fix: dark theme konsisten, layout rapi, data real DB

import { useEffect, useMemo, useRef, useState } from 'react'
import type { Map as LeafletMap } from 'leaflet'
import { createClient } from '@supabase/supabase-js'
import {
  Users, Trophy, Target, CheckCircle, Clock,
  AlertTriangle, Zap, Shield, ChevronRight,
  Award, RefreshCw, Info, Search, X,
  Flame, FileText, Download, Monitor, Map,
} from 'lucide-react'
...
// ── Peta Sebaran ──────────────────────────────────────────
function PetaSebaran({ pins }: { pins: PinData[] }) {
  ...
  useEffect(()=>{
    ...
    async function init() {
      ...
      const map = L.map(mapDiv.current, { center:[-7.0,107.5], zoom:8, zoomControl:true, scrollWheelZoom:false, attributionControl:false })
      mapRef.current = map
      ...
      pins.forEach(pin=>{
        ...
        L.circleMarker([pin.lat,pin.lng],{radius:radius+10,fillColor:color,color:color,weight:0,opacity:0,fillOpacity:0.08}).addTo(map)
        L.circleMarker([pin.lat,pin.lng],{radius,fillColor:color,color:'#020617',weight:2,opacity:1,fillOpacity:0.85})
          .addTo(map)
          .bindPopup(`<div style="font-family:system-ui;min-width:180px;padding:4px">
            <div style="font-size:9px;font-weight:800;color:${color};text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">${pin.isLokal?'● LOKAL':'⚠ NON-LOKAL'}</div>
            ...
            <div style="background:#0a0f1a;border-radius:8px;padding:8px;border:1px solid #1e293b">
              <div style="display:flex;justify-content:space-between;font-size:12px;padding-bottom:6px;margin-bottom:6px;border-bottom:1px solid #1e293b">
                <span style="color:#6b7280">Total</span><b style="color:#fff;font-size:15px">${pin.total}</b>
              </div>
              <div style="display:flex;gap:16px;font-size:11px">
                <span style="color:${ACCENT}">⚡ ${pin.putra}L</span>
                <span style="color:#f472b6">♀ ${pin.putri}P</span>
              </div>
            </div>
          </div>`,{className:'porprov-popup'})
      })

      const LegendControl = L.Control.extend({
        onAdd() {
          const d = L.DomUtil.create('div')
          d.innerHTML=`<div style="background:rgba(2,6,23,0.92);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:10px 12px;backdrop-filter:blur(8px)">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;font-family:system-ui;font-size:11px;color:#d1d5db">
              <div style="width:10px;height:10px;border-radius:50%;background:${ACCENT}"></div> Lokal KBD
            </div>
            <div style="display:flex;align-items:center;gap:8px;font-family:system-ui;font-size:11px;color:#d1d5db">
              <div style="width:10px;height:10px;border-radius:50%;background:#f87171"></div> Non-Lokal
            </div>
          </div>`
          return d
        }
  ...
  return (
    <>
      <style>{`
        ...
        .leaflet-container{background:#020617;font-family:system-ui}
        .leaflet-control-zoom{border:1px solid rgba(255,255,255,0.1)!important;border-radius:8px!important;overflow:hidden}
        .leaflet-control-zoom a{background:rgba(2,6,23,0.9)!important;color:#9ca3af!important}
        .leaflet-control-zoom a:hover{background:rgba(77,166,255,0.1)!important;color:${ACCENT}!important}
      `}</style>
      <div ref={mapDiv} style={{width:'100%',height:'100%',background:'#020617'}}/>
    </>
  )
}
...
export default function DashboardKabBandung() {
  ...
  useEffect(()=>{
    async function load() {
      const [a,k,m] = await Promise.allSettled([
        sb.from('atlet').select('status_registrasi,gender,cabor_nama_raw,kode_asal_daerah,nama_asal_daerah,tgl_lahir').eq('kontingen_id',KONTINGEN_ID),
        sb.from('klasemen_medali').select('emas,perak,perunggu,total,kontingen(nama)').order('emas',{ascending:false}).order('perak',{ascending:false}).limit(10),
      ...
      if(a.status==='fulfilled'&&a.value.data){
        ...
        const pinsData:PinData[]=Object.entries(asalMap).map(([nama,s])=>{
          const coords=ASAL_COORDS[nama]; if(!coords)return null
          return{nama,kode:s.kode,total:s.total,putra:s.putra,putri:s.putri,lat:coords[0],lng:coords[1],isLokal:s.kode.startsWith(KODE_LOKAL)}
        }).filter(Boolean) as PinData[]
        setPins(pinsData)
      }
      if(k.status==='fulfilled'&&k.value.data)setKlasemen(k.value.data as any[])
      if(m.status==='fulfilled'&&m.value.data){const d=m.value.data;setMyMedali({emas:d.emas??0,perak:d.perak??0,perunggu:d.perunggu??0,total:d.total??0})}
      setLoading(false)
    }
    void load()
  },[])
  ...
  const kpi = useMemo(()=>{
    const total=atlets.length,putra=atlets.filter(a=>a.gender==='L').length,putri=atlets.filter(a=>a.gender==='P').length
    ...
    const myRank=klasemen.findIndex(k=>String((k.kontingen as any)?.nama??'').includes('Kab. Bandung'))+1
    return{total,putra,putri,verified,pending,ditolak,lokal,nonLokal,
      vpct:total>0?Math.round(verified/total*100):0,
      lpct:total>0?Math.round(lokal/total*100):0,myRank}
  },[atlets,klasemen])
  ...
  if(loading) return (
    <div className="min-h-screen flex items-center justify-center" style={{background:'#020617'}}>
      <div className="text-center">
        <div className="w-10 h-10 border-2 rounded-full animate-spin mx-auto mb-3" style={{borderColor:`${ACCENT}20`,borderTopColor:ACCENT}}/>
  ...
  return (
    <div className="min-h-screen text-zinc-300" style={{background:'linear-gradient(135deg,#020617 0%,#0a0f1a 100%)'}}>
      ...
      <nav className="sticky top-0 z-50 flex items-center justify-between px-6 py-4 border-b backdrop-blur-xl"
        style={{background:'rgba(2,6,23,0.93)',borderColor:`${ACCENT}12`}}>
        <div className="flex items-center gap-4">
          <div className="w-11 h-11 rounded-xl flex items-center justify-center"
            style={{background:`${PRIMARY}30`,border:`1px solid ${ACCENT}30`,boxShadow:`0 0 20px ${ACCENT}10`}}>
            <span className="text-white font-black text-sm">KBD</span>
          </div>
          <div>
            <h1 className="text-white font-black text-base tracking-wide">DASHBOARD KAB. BANDUNG</h1>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl" style={{background:`${ACCENT}08`,border:`1px solid ${ACCENT}20`}}>
      ...
      <main className="p-5 max-w-[1600px] mx-auto space-y-5 relative z-10">
        ...
        <div {...ani(0)} className="p-4 rounded-2xl flex items-start gap-4"
          ...
          <p className="text-sm text-zinc-300 leading-relaxed">
            ...
            {kpi.nonLokal>0&&<><strong className="text-rose-400">{kpi.nonLokal} non-lokal</strong> ({Math.round(kpi.nonLokal/kpi.total*100)}%) · </>}
        ...
        <div {...ani(40)} className="grid grid-cols-4 gap-3">
          <div className="col-span-2 rounded-2xl p-5 relative overflow-hidden"
            ...
            <div className="text-[10px] text-zinc-500 uppercase tracking-wider mb-3">Total Atlet · {cabors.length} Cabor</div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <div className="flex justify-between text-[10px] mb-1"><span style={{color:ACCENT}}>Putra {kpi.putra}</span><span className="text-white font-bold">{kpi.total>0?Math.round(kpi.putra/kpi.total*100):0}%</span></div>
                <Bar value={kpi.putra} max={kpi.total} color={ACCENT}/>
              </div>
              <div>
                <div className="flex justify-between text-[10px] mb-1"><span className="text-pink-400">Putri {kpi.putri}</span><span className="text-white font-bold">{kpi.total>0?Math.round(kpi.putri/kpi.total*100):0}%</span></div>
          ...
          <div className="rounded-2xl p-4" style={{background:'rgba(255,255,255,0.025)',border:'1px solid rgba(255,255,255,0.07)'}}>
            ...
            {[{l:'Verified',v:kpi.verified,c:'#4ade80'},{l:'Pending',v:kpi.pending,c:'#fbbf24'},{l:'Ditolak',v:kpi.ditolak,c:'#f87171'}].map(s=>(
              <div key={s.l} className="mb-2.5">
                <div className="flex justify-between text-[10px] mb-1"><span style={{color:s.c}}>{s.l}</span><span className="text-white font-mono font-bold">{s.v}</span></div>
          ...
          <div className="rounded-2xl p-4" style={{background:'rgba(255,255,255,0.025)',border:'1px solid rgba(255,215,0,0.12)'}}>
            ...
            <div className="flex gap-1.5">
              {[{l:'Emas',v:myMedali.emas,c:'#ffd700'},{l:'Perak',v:myMedali.perak,c:'#c0c0c0'},{l:'Perunggu',v:myMedali.perunggu,c:'#cd7f32'}].map(m=>(
                <div key={m.l} className="flex-1 text-center rounded-xl py-2" style={{background:`${m.c}12`,border:`1px solid ${m.c}25`}}>
        ...
        <div {...ani(80)} className="grid grid-cols-3 gap-5" style={{alignItems:'start'}}>

          {/* PETA */}
          <div className="col-span-2 rounded-2xl overflow-hidden relative" style={{border:`1px solid ${ACCENT}15`,height:280}}>
            <div className="absolute top-3 left-3 z-[400] pointer-events-none"
              style={{background:'rgba(2,6,23,0.9)',border:`1px solid ${ACCENT}25`,borderRadius:10,padding:'8px 12px',backdropFilter:'blur(8px)'}}>
              <div className="flex items-center gap-2 text-white text-xs font-bold"><Map size={13} style={{color:ACCENT}}/> Radar Sebaran Atlet</div>
              <div className="text-[10px] mt-0.5" style={{color:'rgba(255,255,255,0.35)'}}>
                {pins.filter(p=>p.isLokal).reduce((a,p)=>a+p.total,0)} lokal · {pins.filter(p=>!p.isLokal).reduce((a,p)=>a+p.total,0)} non-lokal
              </div>
            </div>
            <div className="absolute top-3 right-3 z-[400] pointer-events-none flex gap-2">
              <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg" style={{background:'rgba(77,166,255,0.15)',border:`1px solid ${ACCENT}30`}}>
                <div className="w-1.5 h-1.5 rounded-full" style={{background:ACCENT}}/>
                <span className="text-[10px] font-bold" style={{color:ACCENT}}>{kpi.lpct}% Lokal</span>
              </div>
              <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg" style={{background:'rgba(248,113,113,0.12)',border:'1px solid rgba(248,113,113,0.25)'}}>
                <div className="w-1.5 h-1.5 rounded-full bg-red-400"/>
                <span className="text-[10px] font-bold text-red-400">{kpi.nonLokal} Non-Lokal</span>
              </div>
            </div>
            <PetaSebaran pins={pins}/>
          </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="rounded-2xl p-4" style={{background:'rgba(255,255,255,0.025)',border:'1px solid rgba(255,215,0,0.1)'}}>
              <div className="flex items-center gap-2 mb-3"><Trophy size={13} style={{color:'#ffd700'}}/><span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider">Klasemen PORPROV XV</span></div>
              <div className="space-y-1.5">
                {klasemen.slice(0,5).map((k:any,i)=>{
                  const nama=(k.kontingen as any)?.nama??'-'
                  const isUs=nama.includes('Kab. Bandung')&&!nama.includes('Barat')&&!nama.includes('Kota')
                  return(
                    <div key={i} className="flex items-center gap-2 p-2 rounded-xl"
                      ...
                      <span className="text-[10px] w-4 text-center font-mono flex-shrink-0" style={{color:i===0?'#ffd700':i===1?'#c0c0c0':i===2?'#cd7f32':'rgba(255,255,255,0.2)'}}>{i===0?'🥇':i===1?'🥈':i===2?'🥉':`${i+1}`}</span>
                      <span className="flex-1 text-[11px] font-medium truncate" style={{color:isUs?ACCENT:'rgba(255,255,255,0.55)'}}>{nama}{isUs?' ✦':''}</span>
                      <div className="flex gap-1 text-[10px] font-mono font-bold flex-shrink-0">
                        <span style={{color:'#ffd700'}}>{k.emas}</span><span style={{color:'#9ca3af'}}>{k.perak}</span><span style={{color:'#cd7f32'}}>{k.perunggu}</span>
                      </div>
                    </div>
                  )
                })}
            ...
            <div className="rounded-2xl p-4" style={{background:'rgba(255,255,255,0.025)',border:`1px solid ${ACCENT}12`}}>
              <div className="flex items-center gap-2 mb-3"><Target size={13} style={{color:ACCENT}}/><span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider">Target vs Realisasi</span></div>
              {[{l:'Emas',v:myMedali.emas,t:50,c:'#ffd700'},{l:'Perak',v:myMedali.perak,t:40,c:'#c0c0c0'},{l:'Perunggu',v:myMedali.perunggu,t:30,c:'#cd7f32'}].map(m=>(
                <div key={m.l} className="mb-2.5">
                  <div className="flex justify-between text-[10px] mb-1"><span style={{color:m.c}} className="font-bold">{m.l}</span><span className="text-zinc-500 font-mono">{m.v}/{m.t} ({Math.round(m.v/m.t*100)}%)</span></div>
                  <Bar value={m.v} max={m.t} color={m.c} h={5}/>
                </div>
              ))}
            </div>
            ...
            {/* Kemandirian */}
            <div className="rounded-2xl p-4" style={{background:'rgba(255,255,255,0.025)',border:'1px solid rgba(255,255,255,0.07)'}}>
              <div className="flex items-center gap-2 mb-2"><Shield size={13} style={{color:ACCENT}}/><span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider">Kemandirian</span></div>
              <div className="flex items-baseline gap-2 mb-2"><div className="text-3xl font-light text-white">{kpi.lpct}%</div><div className="text-[10px] text-zinc-500">Atlet Lokal</div></div>
              <Bar value={kpi.lokal} max={kpi.total} color={ACCENT} h={5}/>
              <div className="flex justify-between text-[10px] mt-1.5"><span style={{color:ACCENT}}>{kpi.lokal} lokal</span><span className="text-rose-400">{kpi.nonLokal} non-lokal</span></div>
              {kpi.nonLokal>kpi.total*0.15&&(
                <div className="mt-2 flex items-start gap-1.5 p-2 rounded-lg" style={{background:'rgba(248,113,113,0.08)',border:'1px solid rgba(248,113,113,0.2)'}}>
                  <AlertTriangle size={10} style={{color:'#f87171',flexShrink:0,marginTop:1}}/>
                  <span className="text-[9px] text-rose-400">Perlu validasi SK mutasi untuk {kpi.nonLokal} atlet</span>
                </div>
              )}
            </div>
          </div>
        ...
        {/* INTEL CABOR TABS */}
        <div {...ani(120)} className="rounded-2xl overflow-hidden" style={{background:'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.07)'}}>
          <div className="flex items-center justify-between px-5 py-4 border-b" style={{borderColor:'rgba(255,255,255,0.07)'}}>
            <div className="flex gap-1">
              {[{k:'ranking',l:'🏆 Ranking'},{k:'heatmap',l:'🗂 Heatmap'},{k:'conversion',l:'🔥 Conversion'},{k:'gender',l:'👥 Gender'}].map(tab=>(
                <button key={tab.k} onClick={()=>setActiveTab(tab.k as any)}
                  className="px-3.5 py-2 rounded-xl text-xs font-bold transition-all"
                  style={{background:activeTab===tab.k?`${ACCENT}18`:'transparent',color:activeTab===tab.k?ACCENT:'rgba(255,255,255,0.3)',border:activeTab===tab.k?`1px solid ${ACCENT}30`:'1px solid transparent'}}>
                  {tab.l}
                </button>
              ))}
            </div>
            ...
            <div className="max-h-[480px] overflow-y-auto" style={{scrollbarWidth:'thin',scrollbarColor:`${ACCENT}25 transparent`}}>
              {displayed.map((c,i)=>{
                ...
                return(
                  <div key={c.nama} onClick={()=>setSelCabor(p=>p?.nama===c.nama?null:c)}
                    ...
                    <div className="w-36 flex-shrink-0"><div className="text-sm font-bold text-zinc-200 truncate">{c.nama}</div><div className="text-[9px] text-zinc-600">{c.kategori}</div></div>
                    <div className="flex-1"><div className="flex justify-between text-[10px] mb-1"><span className="text-zinc-500">Atlet</span><span className="text-white font-bold">{c.total}</span></div><Bar value={c.total} max={maxAtlet} color={ACCENT}/></div>
                    <div className="w-24 flex-shrink-0">
                      <div className="flex justify-between text-[10px] mb-1"><span style={{color:ACCENT}} className="font-mono">{c.putra}L</span><span className="text-pink-400 font-mono">{c.putri}P</span></div>
                      <div className="h-1.5 rounded-full flex overflow-hidden" style={{background:'rgba(255,255,255,0.06)'}}><div style={{width:`${c.total>0?c.putra/c.total*100:50}%`,background:ACCENT}}/><div style={{flex:1,background:'#f472b6'}}/></div>
                    </div>
                    <div className="w-16 text-center flex-shrink-0"><div className="text-sm font-bold text-green-400">{c.verified}</div><div className="text-[9px] text-zinc-600">verified</div></div>
                    <div className="w-24 flex-shrink-0 text-right">
                      <div className="text-xs font-bold"><span className="text-yellow-400">🥇{c.emas}</span>{' '}<span className="text-zinc-400">🥈{c.perak}</span>{' '}<span style={{color:'#cd7f32'}}>🥉{c.perunggu}</span></div>
                    ...
                    <div className="w-28 flex-shrink-0 flex justify-end">
                      <span className="text-[9px] font-bold px-2 py-1 rounded-lg"
                        style={c.emas>0?{background:`${ACCENT}12`,color:ACCENT,border:`1px solid ${ACCENT}25`}:c.conversion>15?{background:'rgba(251,191,36,0.1)',color:'#fbbf24',border:'1px solid rgba(251,191,36,0.2)'}:{background:'rgba(255,255,255,0.04)',color:'rgba(255,255,255,0.2)',border:'1px solid rgba(255,255,255,0.07)'}}>
              ...
              {!showAll&&filtered.length>12&&(
                <div className="p-4 flex justify-center border-t" style={{borderColor:'rgba(255,255,255,0.05)'}}>
                  <button onClick={()=>setShowAll(true)} className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-bold"
          ...
          {activeTab==='heatmap'&&(
            <div className="p-5 grid grid-cols-4 gap-3">
              {byKategori.map(([kat,d])=>{
                ...
                return(
                  <div key={kat} className="rounded-xl p-4" style={{background:`${col}08`,border:`1px solid ${col}20`}}>
                    <div className="flex justify-between mb-2"><span className="text-xs font-bold" style={{color:col}}>{kat}</span><span className="text-[9px] text-zinc-600">{d.list.length} cabor</span></div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                      {d.list.slice(0,3).map(c=><span key={c} className="text-[8px] px-1.5 py-0.5 rounded" style={{background:'rgba(255,255,255,0.05)',color:'rgba(255,255,255,0.3)'}}>{c}</span>)}
          ...
          {activeTab==='conversion'&&(
            <div className="p-5">
              <div className="flex items-center gap-2 text-xs text-zinc-500 mb-5 p-3 rounded-xl" style={{background:'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.06)'}}>
                <Info size={12}/><span><strong style={{color:ACCENT}}>Medal Conversion Rate</strong> = Total Medali ÷ Total Atlet × 100. KPI efisiensi pembinaan.</span>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {topConv.map(c=>{
                  ...
                  return(
                    <div key={c.nama} className="flex items-center gap-3 p-4 rounded-xl" style={{background:'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.05)'}}>
                      <div className="w-9 h-9 rounded-xl flex items-center justify-center font-black text-sm flex-shrink-0" style={{background:`${g.c}20`,color:g.c}}>{g.l}</div>
                      ...
                      <div className="text-right flex-shrink-0"><div className="text-xl font-black" style={{color:g.c}}>{c.conversion}%</div><div className="text-[9px] text-zinc-600">{tm} medali</div></div>
                    </div>
                  )
                })}
              </div>
          ...
          {activeTab==='gender'&&(
            <div className="p-5 grid grid-cols-2 gap-6">
              <div>
                <div className="flex items-center gap-2 mb-4"><div className="w-2 h-2 rounded-full" style={{background:ACCENT}}/><span className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Dominan Putra (80%+)</span></div>
                <div className="space-y-2.5">
                  {domPutra.map(c=>(
                    <div key={c.nama} className="flex items-center gap-3 p-3 rounded-xl" style={{background:'rgba(255,255,255,0.02)',border:`1px solid ${ACCENT}12`}}>
                      <div className="flex-1"><div className="text-sm font-bold text-zinc-200">{c.nama}</div><div className="text-[10px] text-zinc-500 mt-0.5">{c.putra}L / {c.putri}P</div><div className="h-1.5 rounded-full flex overflow-hidden mt-1.5" style={{background:'rgba(255,255,255,0.06)'}}><div style={{width:`${c.putra/c.total*100}%`,background:ACCENT}}/><div style={{flex:1,background:'#f472b6'}}/></div></div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-4"><div className="w-2 h-2 rounded-full bg-pink-400"/><span className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Dominan Putri (60%+)</span></div>
                <div className="space-y-2.5">
                  {domPutri.map(c=>(
                    <div key={c.nama} className="flex items-center gap-3 p-3 rounded-xl" style={{background:'rgba(255,255,255,0.02)',border:'1px solid rgba(244,114,182,0.12)'}}>
                      <div className="flex-1"><div className="text-sm font-bold text-zinc-200">{c.nama}</div><div className="text-[10px] text-zinc-500 mt-0.5">{c.putra}L / {c.putri}P</div><div className="h-1.5 rounded-full flex overflow-hidden mt-1.5" style={{background:'rgba(255,255,255,0.06)'}}><div style={{width:`${c.putra/c.total*100}%`,background:ACCENT}}/><div style={{flex:1,background:'#f472b6'}}/></div></div>
        ...
        <div {...ani(160)} className="grid grid-cols-3 gap-4">
          {[
            {l:'War Room',       d:'Pemantauan laga & prediksi medali',    icon:Monitor,  c:ACCENT,    href:'/konida/warroom/kabbandung'},
            {l:'Lap. Harian',    d:'Jurnal hasil pertandingan per hari',   icon:FileText, c:'#60a5fa', href:'/konida/lappertandingan/kabbandung'},
            {l:'Premium Report', d:'SPJ bonus, buku hasil, piagam juara', icon:Download, c:'#f59e0b', href:'/konida/Premiumreport/kabbandung'},
          ].map(a=>(
            <a key={a.l} href={a.href}
              ...
              <div className="flex items-center gap-4">
                <div className="p-3 rounded-xl" style={{background:`${a.c}12`,border:`1px solid ${a.c}20`}}><a.icon size={20} style={{color:a.c}}/></div>
                <div><div className="font-bold text-white text-sm">{a.l}</div><div className="text-[11px] text-zinc-500 mt-0.5">{a.d}</div></div>
              </div>
      ...
      {selCabor&&(
        <div className="fixed inset-0 z-50" onClick={()=>setSelCabor(null)}>
          ...
          <div className="absolute right-0 top-0 h-full w-96 overflow-y-auto" style={{background:'#040f08',borderLeft:`1px solid ${ACCENT}20`}} onClick={e=>e.stopPropagation()}>
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <div><div className="text-[10px] uppercase tracking-wider mb-1" style={{color:ACCENT}}>Detail Cabor</div><h2 className="text-2xl font-black text-white">{selCabor.nama}</h2><div className="text-xs text-zinc-500 mt-0.5">{selCabor.kategori}</div></div>
                <button onClick={()=>setSelCabor(null)} style={{color:'rgba(255,255,255,0.3)'}}><X size={18}/></button>
              </div>
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  {[{l:'Total',v:selCabor.total,c:ACCENT},{l:'Verified',v:selCabor.verified,c:'#4ade80'},{l:'Putra',v:selCabor.putra,c:ACCENT},{l:'Putri',v:selCabor.putri,c:'#f472b6'}].map(s=>(
                ...
                <div className="rounded-xl p-4" style={{background:'rgba(255,215,0,0.06)',border:'1px solid rgba(255,215,0,0.15)'}}>
                  ...
                  <div className="flex gap-3">
                    {[{l:'Emas',v:selCabor.emas,c:'#ffd700'},{l:'Perak',v:selCabor.perak,c:'#c0c0c0'},{l:'Perunggu',v:selCabor.perunggu,c:'#cd7f32'}].map(m=>(
                      <div key={m.l} className="flex-1 text-center"><div className="text-2xl font-black" style={{color:m.c}}>{m.v}</div><div className="text-[10px] text-zinc-500">{m.l}</div></div>
                    ))}
                  </div>
                ...
                <div className="rounded-xl p-4" style={{background:'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.06)'}}>
                  ...
                  <p className="text-xs text-zinc-300 leading-relaxed">
                    {selCabor.conversion>=50?'✅ Efisiensi sangat baik — cabor unggulan.':selCabor.conversion>=25?'🔥 Efisiensi baik, ada peluang meningkat.':selCabor.total>30?'⚠️ Banyak atlet tapi konversi rendah.':'📋 Data terbatas, perlu monitoring.'}
                  </p>
                </div>
              </div>
            </div>
      ...
      <style>{`*{box-sizing:border-box}::-webkit-scrollbar{width:4px;height:4px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:${ACCENT}30;border-radius:4px}`}</style>
    </div>
  )
}