'use client'
// src/app/login/kabbogor/page.tsx
// Kab. Bogor Enterprise Login v2 (Refactored & Solid Dark Theme)
// Background: peta wilayah Kab. Bogor + glass morphism form (Solid)
...
const STATS = [
  ...
  { label: 'Cabang Olahraga', value: '54',    color: 'text-amber-400',   icon: '🏅' },
  { label: 'Total Venue',     value: '10',    color: 'text-blue-400',    icon: '🏟️' },
  { label: 'Kecamatan',       value: '40',    color: 'text-purple-400',  icon: '📍' },
]
...
export default function LoginKabBogor() {
  ...
  return (
    <div className="min-h-screen flex overflow-hidden font-sans bg-[#020d06]">
      ...
      {/* ── KIRI: Peta Background ── */}
      <div className="hidden lg:flex w-[58%] relative overflow-hidden flex-col p-10 justify-between">
        
        {/* Background Layers */}
        <div className="absolute inset-0 bg-[url('/logos/peta_kabbogor.jpg')] bg-cover bg-center saturate-[0.4] brightness-[0.35]" />
        <div className="absolute inset-0 bg-gradient-to-br from-[#020d06]/75 via-[#061408]/60 to-[#020d06]/85" />
        <div className="absolute inset-0" style={{ backgroundImage: 'linear-gradient(rgba(52,211,153,0.06) 1px, transparent 1px), linear-gradient(90deg, rgba(52,211,153,0.06) 1px, transparent 1px)', backgroundSize: '40px 40px' }}/>
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,transparent_30%,rgba(2,13,6,0.7)_100%)]" />
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-transparent to-[#020d06]/95" />
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center">
          ...
          <p className="text-white/40 text-[13px] leading-relaxed max-w-[320px] font-medium">
          ...
          <div className="flex gap-4 mt-8 p-4 bg-[#060f08]/60 rounded-xl border border-emerald-400/10 backdrop-blur-md max-w-md">
            {[
              ...
              <div key={s.l} className="text-center flex-1 border-r border-white/5 last:border-0">
                ...
                <div className="text-white/30 text-[9px] mt-1 font-medium tracking-wide uppercase">{s.l}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="relative z-10 flex flex-col gap-4">
          <div className="grid grid-cols-4 gap-3">
            {STATS.map(s => (
              <div key={s.label} className="p-3 rounded-xl bg-[#060f08]/75 border border-emerald-400/10 backdrop-blur-md hover:bg-[#060f08]/90 transition-colors">
                ...
                <div className="text-white/30 text-[8px] uppercase tracking-[0.08em] mt-1 font-semibold">{s.label}</div>
              </div>
            ))}
          </div>

          <div className="flex flex-col gap-1.5">
            <div className="text-white/20 text-[9px] font-bold tracking-[0.15em] uppercase mb-1">
              Venue Unggulan
            </div>
            {VENUES.map((v, i) => (
              <div key={v.nama} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border-l-2 transition-all ${
                ...
                <div className={`flex-1 text-[10px] font-medium ${i < 2 ? 'text-slate-200' : 'text-white/30'}`}>{v.nama}</div>
          ...
          <div className="flex items-center justify-between px-3 py-2.5 rounded-xl bg-[#060f08]/50 border border-emerald-400/10 backdrop-blur-sm mt-2">
            <span className="text-white/25 text-[9px] font-bold tracking-wider">ATLET TERDAFTAR</span>
            <AtletTicker/>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center relative p-8 bg-gradient-to-br from-[#020d06] via-[#041209] to-[#020d06]">
        ...
        {/* Glassmorphism Card Wrapper - (Dibuat jauh lebih solid/tebal di sini) */}
        <div className="relative z-10 w-full max-w-[380px] p-8 rounded-3xl bg-[#051408]/95 backdrop-blur-xl border border-emerald-700/60 shadow-2xl shadow-black">
          ...
          <div className="flex flex-col items-center mb-8 lg:hidden">
            <div className="w-14 h-14 rounded-2xl mb-3 bg-emerald-900/20 border-2 border-emerald-400/30 flex items-center justify-center shadow-[0_0_15px_rgba(52,211,153,0.15)]">
          ...
          <p className="text-white/70 text-xs mb-8 font-medium"> {/* Dipertegas opacity-nya */}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            {/* Username */}
            <div>
              <label htmlFor="username" className="text-white/70 text-[10px] font-bold tracking-[0.15em] uppercase block mb-2">Username</label> {/* Label dipertegas */}
              <input 
                id="username"
                type="text" 
                value={username} 
            ...
            {/* Password */}
            <div>
              <label htmlFor="password" className="text-white/70 text-[10px] font-bold tracking-[0.15em] uppercase block mb-2">Password</label> {/* Label dipertegas */}
            ...
            <div className="mt-2 rounded-xl p-3 bg-[#030a05]/60 border border-emerald-400/20 group hover:border-emerald-400/40 transition-colors">
              ...
              <button 
                ...
                className="w-full flex items-center justify-between p-2 rounded-lg hover:bg-emerald-400/10 transition-colors group-hover:bg-emerald-400/5"
              >
            ...
            <button 
              ...
              className={`w-full mt-4 py-3.5 rounded-xl text-[13px] font-bold uppercase tracking-[0.06em] flex items-center justify-center gap-2.5 transition-all duration-300 ${
                loading || !isFormValid 
                  ? 'bg-[#1e293b] text-white/50 border border-slate-700 cursor-not-allowed' // Diperjelas status disabled-nya
