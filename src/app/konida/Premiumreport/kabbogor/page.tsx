'use client'
// src/app/konida/Premiumreport/kabbogor/page.tsx — v4
...
const KONTINGEN_NAME = 'Kab. Bogor'
...
function renderMarkdown(text: string): string {
  ...
  html = html.replace(/```([\s\S]*?)```/g, (_, code) =>
    `<pre style="background:rgba(0,0,0,0.4);padding:12px;border-radius:8px;margin:12px 0;overflow-x:auto;font-size:11px;border:1px solid rgba(0,255,170,0.15);color:#00ffaa;font-family:monospace;">${code.trim()}</pre>`
  )
  ...
  html = html
    ...
    .replace(/^## (.+)$/gm, '<h2 style="font-size:18px;font-weight:800;color:#00ffaa;margin:24px 0 10px;padding-bottom:6px;border-bottom:1px solid rgba(0,255,170,0.2);">$1</h2>')
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith('<h') || trimmed.startsWith('<hr') || trimmed.startsWith('<pre')) {
      ...
      result.push(`<li style="margin:6px 0;color:#cbd5e1;line-height:1.6;font-size:13px;list-style:none;position:relative;padding-left:18px;">
        <span style="position:absolute;left:0;color:#00ffaa;">▸</span>${content}
      </li>`)
    } else if (trimmed === '') {
      flushParagraph()
      flushList()
...
export default function PagePremiumReport() {
  ...
  function printBrief() {
    ...
    <style>
      ...
      h1{font-size:22px;color:#065f46;margin-bottom:8px;border-bottom:2px solid #065f46;padding-bottom:6px}
      h2{font-size:16px;color:#065f46;margin:20px 0 8px;border-bottom:1px solid #d1fae5;padding-bottom:4px}
      ...
      .header{padding:12px 16px;background:#f0fdf4;border:1px solid #065f46;border-radius:8px;margin-bottom:20px}
      @media print{button{display:none}}
    </style></head><body>
    <button onclick="window.print()" style="margin-bottom:16px;padding:8px 16px;background:#065f46;color:#fff;border:none;border-radius:6px;cursor:pointer">🖨 Print / Save as PDF</button>
    <div class="header">
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:'Segoe UI',Arial,sans-serif;background:#020d06;color:#fff}
  .slide{width:297mm;height:210mm;padding:18mm 25mm;position:relative;background:linear-gradient(135deg,#020d06 0%,#061a10 100%);page-break-after:always;overflow:hidden}
  .slide:last-child{page-break-after:auto}
  .grid-bg{position:absolute;inset:0;opacity:0.05;background-image:linear-gradient(#00ffaa 1px,transparent 1px),linear-gradient(90deg,#00ffaa 1px,transparent 1px);background-size:30px 30px;pointer-events:none}
  h1{font-size:38px;color:#00ffaa;font-weight:900;letter-spacing:-1px;margin-bottom:6px}
  h2{font-size:24px;color:#fff;font-weight:800;margin-bottom:16px;padding-bottom:8px;border-bottom:2px solid #00ffaa40}
  ...
  .kpi{background:rgba(0,255,170,0.05);border:1px solid #00ffaa20;border-radius:12px;padding:16px}
  .kpi .num{font-size:36px;font-weight:900;color:#00ffaa;line-height:1}
  .kpi .lbl{font-size:10px;color:#94a3b8;text-transform:uppercase;margin-top:4px}
  .footer{position:absolute;bottom:10mm;left:25mm;right:25mm;display:flex;justify-content:space-between;font-size:9px;color:#4b5563;border-top:1px solid #00ffaa15;padding-top:8px}
  ...
  .badge-emerald{background:rgba(16,185,129,0.15);color:#10b981;border:1px solid #10b98130}
  table{width:100%;border-collapse:collapse;margin-top:12px}
  th{background:rgba(0,255,170,0.1);color:#00ffaa;padding:8px 10px;text-align:left;font-size:10px;text-transform:uppercase;border-bottom:1px solid #00ffaa30}
  ...
  .strip-green{padding:12px 16px;background:rgba(16,185,129,0.08);border-left:4px solid #10b981;border-radius:0 8px 8px 0;margin:8px 0;font-size:11px;color:#6ee7b7}
  .print-btn{position:fixed;top:20px;right:20px;background:#00ffaa;color:#020d06;border:none;padding:12px 24px;border-radius:8px;font-weight:700;cursor:pointer;z-index:1000}
...
<div class="slide"><div class="grid-bg"></div>
  ...
  <table><thead><tr><th>Kategori</th><th>Target</th><th>Prediksi</th><th>Gap</th><th>Confidence</th></tr></thead><tbody>
    ...
    <tr><td><strong class="bronze">🥉 Perunggu</strong></td><td>${TARGET_BUPATI.perunggu}</td><td>${Math.round(TARGET_BUPATI.perunggu*0.9)}</td><td style="color:#10b981">+${Math.round(TARGET_BUPATI.perunggu*0.9)-TARGET_BUPATI.perunggu}</td><td>90%</td></tr>
  </tbody></table>
...
<div class="slide"><div class="grid-bg"></div>
  ...
  ${topAtlet.map((a,i)=>`<tr><td><strong style="color:${i<3?'#fbbf24':'#94a3b8'}">${i+1}</strong></td><td><strong>${a.nama_lengkap}</strong></td><td>${a.cabor_nama_raw}</td><td><strong style="color:#00ffaa">${tesFisikMap[a.id]?.kesimpulan_persen||0}%</strong></td><td>${tesFisikMap[a.id]?.kesimpulan_kategori||'-'}</td></tr>`).join('')}
  </tbody></table>
...
<div class="slide"><div class="grid-bg"></div>
  ...
  <div class="kpi-grid">
    ...
    <div class="kpi"><div class="num" style="color:#10b981">${stats.skorTinggi}</div><div class="lbl">Elite ≥80%</div></div>
...
<div class="slide"><div class="grid-bg"></div>
  ...
  <table><thead><tr><th>Komponen</th><th>Volume</th><th>Tarif</th><th>Subtotal</th></tr></thead><tbody>
    ...
    <tr style="background:rgba(0,255,170,0.08)"><td colspan="3"><strong style="color:#00ffaa">TOTAL ANGGARAN</strong></td><td><strong style="color:#00ffaa;font-size:14px">Rp ${totalAnggaran.toLocaleString('id')}</strong></td></tr>
  </tbody></table>
...
<div class="slide"><div class="grid-bg"></div>
  <div style="height:100%;display:flex;flex-direction:column;justify-content:center;align-items:center;text-align:center;position:relative;z-index:10">
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:20px;width:100%;max-width:800px">
      <div style="padding:20px;background:rgba(0,255,170,0.05);border:1px solid #00ffaa30;border-radius:12px"><div style="font-size:32px;color:#00ffaa;font-weight:900">H-150</div><div style="font-size:11px;color:#94a3b8">Finalisasi Roster</div></div>
    ...
    <div style="margin-top:40px;padding:14px 40px;border:1px solid #00ffaa40;border-radius:8px;color:#00ffaa;font-size:11px;letter-spacing:2px">TERIMA KASIH</div>
  </div>
  ...
  async function generateMedalProjection() {
    ...
    try {
      ...
      XLSX.writeFile(wb, `Medal_Projection_KabBogor_${new Date().toISOString().slice(0,10)}.xlsx`)
...
<style>
  ...
  .card{width:85.6mm;height:54mm;background:linear-gradient(135deg,#065f46 0%,#064e3b 50%,#022c22 100%);border-radius:3mm;padding:3mm;color:#fff;position:relative;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.15);display:flex}
  .card::before{content:'';position:absolute;top:0;left:0;right:0;height:1mm;background:linear-gradient(90deg,#00ffaa,#10b981,#00ffaa)}
  .card-wm{position:absolute;right:-10mm;bottom:-10mm;font-size:30mm;opacity:0.08;color:#00ffaa}
  ...
  .org{font-size:6pt;color:#00ffaa;text-transform:uppercase;letter-spacing:1px;font-weight:700;margin-bottom:1mm}
  ...
  .badge{display:inline-block;padding:0.5mm 1.5mm;background:rgba(0,255,170,0.2);border:0.3mm solid #00ffaa;border-radius:0.8mm;font-size:6pt;color:#00ffaa;font-weight:700;text-transform:uppercase;margin-top:1mm}
  ...
  .print-btn{position:fixed;top:20px;right:20px;background:#065f46;color:#fff;border:none;padding:12px 24px;border-radius:8px;font-weight:700;cursor:pointer;z-index:1000}
...
document.addEventListener('DOMContentLoaded',function(){
  ${verified.map((a,idx)=>{const pi=Math.floor(idx/8);const ci=idx%8;return `QRCode.toCanvas(document.getElementById('qr-${pi}-${ci}'),'https://porprov-xv.vercel.app/atlet/${a.id}',{width:80,margin:0,color:{dark:'#065f46',light:'#ffffff'}}).catch(console.error);`}).join('')}
});
</script></body></html>`
    const w = window.open('','_blank'); if (w) { w.document.write(html); w.document.close() }
    setLoadingFile(null)
  ...
  function generateBukuHasil() {
    ...
    const html = `<!DOCTYPE html><html><head><title>Buku Hasil</title><style>body{font-family:Arial,sans-serif;font-size:10px;padding:30px}h1{font-size:16px;text-align:center}table{width:100%;border-collapse:collapse;margin-top:8px}th{background:#065f46;color:#fff;padding:5px 8px;font-size:9px}td{padding:4px 8px;border-bottom:1px solid #e5e7eb;font-size:9px}@media print{button{display:none}}</style></head><body><h1>BUKU HASIL — ${KONTINGEN_NAME}</h1><p style="text-align:center;color:#666">PORPROV XV · 7-20 Nov 2026</p><button onclick="window.print()" style="margin:10px;padding:6px 16px;background:#065f46;color:#fff;border:none;border-radius:6px;cursor:pointer">🖨 Print</button>${totalJurnal===0?'<p style="padding:20px;background:#fef3c7;text-align:center;color:#92400e">Belum ada jurnal diinput</p>':HARI.map(h=>{const laga=jurnalLS[h.hari]||[];if(laga.length===0)return '';return `<h3 style="margin-top:16px;color:#065f46">Hari ke-${h.hari} · ${h.tgl}</h3><table><thead><tr><th>Waktu</th><th>Cabor</th><th>Hasil</th><th>Medali</th></tr></thead><tbody>${laga.map(j=>`<tr><td>${j.waktu||'—'}</td><td><strong>${j.cabor}</strong></td><td>${j.hasil}</td><td>${j.medali==='Emas'?'🥇':j.medali==='Perak'?'🥈':j.medali==='Perunggu'?'🥉':'—'} ${j.medali}</td></tr>`).join('')}</tbody></table>`}).join('')}</body></html>`
    const w = window.open('','_blank'); w?.document.write(html); w?.document.close(); setLoadingFile(null)
  }

  function cetakSertifikat() {
    setLoadingFile('CERT')
    const html = `<!DOCTYPE html><html><head><title>Sertifikat</title><style>body{font-family:'Georgia',serif;margin:0}.page{width:210mm;height:148mm;padding:20mm;border:8px double #065f46;margin:0 auto 20px;background:linear-gradient(135deg,#f0fdf4,#fff);page-break-after:always}h1{font-size:22px;text-align:center;color:#065f46;letter-spacing:2px}.nama{font-size:24px;text-align:center;color:#111;font-weight:800;margin:20px 0;border-bottom:2px solid #065f46;padding-bottom:6px}.medali{font-size:18px;text-align:center;font-weight:800;color:#b45309;margin:12px 0}button{margin:8px;padding:6px 16px;background:#065f46;color:#fff;border:none;border-radius:6px;cursor:pointer}@media print{button{display:none}}</style></head><body><button onclick="window.print()">🖨 Cetak</button>${atlets.slice(0,Math.max(stats.emas,3)).map(a=>`<div class="page"><div style="font-size:40px;text-align:center">🏆</div><h1>PIAGAM PENGHARGAAN</h1><h2 style="font-size:14px;text-align:center;color:#374151;font-weight:normal">PORPROV XV JAWA BARAT 2026</h2><p style="text-align:center;font-size:11px;color:#666;margin-top:16px">Diberikan kepada:</p><div class="nama">${a.nama_lengkap}</div><p style="text-align:center;font-size:13px;color:#065f46">Cabor: <strong>${a.cabor_nama_raw}</strong></p><div class="medali">🥇 MEDALI EMAS</div></div>`).join('')}</body></html>`
    const w = window.open('','_blank'); w?.document.write(html); w?.document.close(); setLoadingFile(null)
  }
    ...
    <div className="min-h-screen text-zinc-300 flex flex-col"
      ...
      <div className="sticky top-0 z-40 border-b backdrop-blur-xl px-6 py-4"
        ...
        <div className="max-w-[1600px] mx-auto flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {[{v:stats.total,l:'Atlet',c:ACCENT},{v:stats.sudahTes,l:'Tested',c:'#10b981'},{v:stats.emas,l:'🥇',c:'#fbbf24'},{v:stats.hasRek,l:'Rek',c:'#60a5fa'}].map(s=>(
      ...
      <main className="flex-1 p-6 max-w-[1600px] w-full mx-auto space-y-6 relative z-10">
        ...
        <div {...ani(0)} className="rounded-3xl p-8 relative overflow-hidden"
          style={{
            background:'linear-gradient(135deg, rgba(168,85,247,0.1) 0%, rgba(0,255,170,0.06) 50%, rgba(59,130,246,0.08) 100%)',
          ...
          <div className="absolute -bottom-20 -left-20 w-96 h-96 rounded-full pointer-events-none"
            style={{background:'radial-gradient(circle, rgba(0,255,170,0.15) 0%, transparent 70%)'}}/>
          ...
          <div className="relative flex items-center gap-6 flex-wrap">
            <div className="w-24 h-24 rounded-3xl flex items-center justify-center flex-shrink-0 relative"
              ...
              <div className="absolute -bottom-1 -left-1 w-6 h-6 rounded-full flex items-center justify-center"
                style={{background:'#10b981', border:'2px solid #020d06'}}>
            ...
            <div className="flex-1 min-w-[300px]">
              <div className="flex items-center gap-2 mb-2 flex-wrap">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded"
                  style={{background:'rgba(0,255,170,0.2)',color:ACCENT,border:`1px solid ${ACCENT}40`}}>
                  POWERED BY CLAUDE
                </span>
              ...
              <p className="text-sm text-zinc-300 leading-relaxed max-w-[700px] mb-1">
                ...
                Claude AI menganalisa <strong style={{color:ACCENT}}>{stats.total} atlet</strong> · <strong style={{color:'#10b981'}}>{stats.sudahTes} tes biomotorik</strong> · <strong style={{color:'#fbbf24'}}>{stats.totalEmas+stats.totalPerak+stats.totalPerunggu} prestasi historic</strong> dalam 30 detik.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-2 flex-shrink-0">
              ...
              <a href="/konida/sipa/kabbogor" target="_blank" rel="noopener"
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-black text-white flex items-center gap-2">
                    AI Strategic Brief
                    {briefStreaming && <span className="text-[10px] font-mono text-emerald-400 animate-pulse">● STREAMING</span>}
                  </div>
              ...
              {hasBrief && !briefStreaming && (
                <div className="flex items-center gap-1.5 flex-wrap">
                  <button onClick={copyBrief}
                    ...
                    style={{background:briefCopied?'rgba(16,185,129,0.15)':'rgba(255,255,255,0.05)', color:briefCopied?'#10b981':'rgba(255,255,255,0.7)', border:`1px solid ${briefCopied?'rgba(16,185,129,0.3)':'rgba(255,255,255,0.1)'}`}}>
        ...
        <div {...ani(40)} className="space-y-3">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="rounded-2xl p-5 flex flex-col" style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(16,185,129,0.25)'}}>
              <div className="flex items-start gap-3 mb-4">
                <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style={{background:'rgba(16,185,129,0.12)',border:'1px solid rgba(16,185,129,0.25)'}}><Target size={22} style={{color:'#10b981'}}/></div>
              ...
              <button onClick={generateMedalProjection} disabled={loadingFile!==null} className="w-full py-3 rounded-xl text-xs font-bold flex items-center justify-center gap-2 disabled:opacity-40" style={{background:'rgba(16,185,129,0.12)',border:'1px solid rgba(16,185,129,0.3)',color:'#6ee7b7'}}>{loadingFile==='PROJECTION'?<Loader2 size={14} className="animate-spin"/>:<Download size={14}/>}{loadingFile==='PROJECTION'?'Computing...':'GENERATE'}</button>
            </div>
