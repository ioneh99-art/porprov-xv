'use client'
// src/app/konida/Premiumreport/kabbandung/heatmap-cabor/page.tsx
// Heatmap Cabor × Komponen Fisik — Kab. Bandung
...
const BACK    = '/konida/Premiumreport/kabbandung'

// ── Helpers ──────────────────────────────────────────────
function getZone(val: number): { label: string; color: string; bg: string } {
  if (val >= 80) return { label: 'Sangat Baik',  color: '#0ea5e9', bg: 'rgba(16,185,129,0.12)'  }
...
function DetailPanel({
  ...
  const deltaColor = delta > 2 ? '#0ea5e9' : delta < -2 ? '#ef4444' : '#fbbf24'
...
export default function HeatmapCaborPage() {
  ...
  useEffect(() => {
    fetch('/api/konida/tes-fisik?kontingen_id=4')
      ...
      <div className="rounded-2xl p-6"
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-300">Data biomotorik Kab. Bandung belum tersedia. Hubungi tim UPI Sport Science.</p>
          </div>
        </div>
      </div>
    </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[10px] font-mono uppercase tracking-widest mt-0.5" style={{ color:PRIMARY }}>
              Kab. Bandung · Sport Science FPOK UPI · Klik sel untuk detail
            </div>
          </div>
        </div>
            ...
            {komponen.length === 0 ? <Empty/> : (
              <>
                ...
                <div className="mt-4 p-3 rounded-xl text-xs"
                  ...
                  Terkuat:{' '}
                  <strong className="text-sky-400">{komponen[komponen.length - 1]?.komponen}</strong> ({komponen[komponen.length - 1]?.rata_capaian}%).
                </div>
              </>
            )}
          </Panel>
                          ...
                          {shown.map((a: any, i: number) => (
                            <tr key={a.id}
                              ...
                              <td className="p-2 text-right font-bold font-mono"
                                style={{ color: (a.kesimpulan_persen||0) >= 80 ? '#0ea5e9'
