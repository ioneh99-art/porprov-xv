...
export default function RekonsiliasiPage() {
  ...
  const perlu = useMemo(() => rows.map((r, i) => ({ r, i })).filter(x => x.r.status_peserta === 'perlu_konfirmasi'), [rows])
  const stat = useMemo(() => ({
  ...
  return (
    <div className="min-h-screen bg-[#eeeeee] p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <p className="text-xs text-gray-400 mb-6">Cocokkan file resmi KONI (Rekapitulasi) dengan database — pisahkan peserta resmi dari kolam atlet. Preview dulu, konfirmasi beda-ejaan, baru simpan.</p>
          ...
          <>
            ...
            {perlu.length > 0 && (
              <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden mb-4">
                <div className="px-4 py-3 border-b border-gray-100 text-sm font-semibold text-[#3c4858]">Panel Konfirmasi — beda ejaan ({perlu.length})</div>
                <div className="max-h-96 overflow-auto divide-y divide-gray-50">
                  {perlu.slice(0, 300).map(({ r, i }) => (
                    <div key={i} className="flex items-center gap-3 px-4 py-2.5 text-sm">
                      ...
                      <button onClick={() => resolve(i, true)} className="flex items-center gap-1 text-emerald-600 hover:bg-emerald-50 rounded-lg px-2.5 py-1.5 text-xs font-medium"><CheckCircle size={13} /> Terima</button>
                      <button onClick={() => resolve(i, false)} className="flex items-center gap-1 text-gray-400 hover:bg-red-50 hover:text-red-500 rounded-lg px-2.5 py-1.5 text-xs"><XCircle size={13} /> Tolak</button>
                    </div>
                  ))}
                </div>
                {perlu.length > 300 && <div className="px-4 py-2 text-[11px] text-gray-400">…{perlu.length - 300} lagi</div>}
              </div>
            )}

            {/* Commit */}
            <div className="flex items-center gap-3">
              <button onClick={commit} disabled={perlu.length > 0 || busy === 'commit'}
              ...
              {perlu.length > 0 && <span className="text-xs text-amber-600">Selesaikan {perlu.length} konfirmasi dulu.</span>}
            </div>
          </>
        )}
      </div>
