...
const ACCENT       = '#38bdf8'
const NAMA_KAB     = 'KABUPATEN BANDUNG'
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-[1000px] rounded-2xl overflow-hidden flex flex-col"
        style={{background:'#03101c',border:`1px solid ${isDemo?'rgba(251,191,36,0.3)':`${ACCENT}20`}`,maxHeight:'85vh',boxShadow:'0 25px 60px rgba(0,0,0,0.8)'}}>
...
export default function PageLaporan() {
  ...
  const analytics = useMemo(()=>{
    ...
    const lokal       = atlets.filter(a=>a.kode_asal_daerah?.startsWith('3204')).length
  ...
  const keuanganAlerts = useMemo<CriticalAlert[]>(() => {
    ...
    if (analytics.nonLokal > 0)
      alerts.push({
        ...
        message: `${analytics.nonLokal} atlet KTP-nya bukan Kab. Bandung — perlu klarifikasi legalitas sebelum SK dicetak`,
    ...
    switch(id) {
      ...
      case 'sk-kontingen': {
        ...
        const data = verified.map(a=>[
          ...
          a.kode_asal_daerah?.startsWith('3204')?'Kab. Bandung (Lokal)':(a.nama_asal_daerah||'-'),
          a.status_registrasi,
        ])
      ...
      case 'audit-nik': {
        ...
        const data = atlets.map(a=>[
          ...
          a.kode_asal_daerah?.startsWith('3204')?'LOKAL':'NON-LOKAL',
      ...
      case 'manifest-akomodasi': {
        ...
        const hotels = ['Hotel Permata','Hotel Pakuan','Hotel Royal Bandung','Wisma KONI']
  ...
  function exportCSV(id: string) {
    ...
    el.href    = url
    el.download= `${id}_kabbandung_${new Date().toISOString().slice(0,10)}.csv`
  ...
  function printLaporan(id: string) {
    ...
    <style>
      ...
      th{background:#075985;color:#fff;padding:5px 8px;text-align:left;font-size:9px;text-transform:uppercase}
    ...
    <button onclick="window.print()" style="margin-bottom:12px;padding:6px 16px;background:#075985;color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:11px">🖨 Print</button>
  ...
  const REPORTS: ReportDef[] = [
    // FEATURED
    {
      ...
      icon:Activity, color:'#0ea5e9', isFeatured:true, isDemo:false,
    ...
    // ADMIN
    {
      ...
      icon:Users, color:'#22d3ee', isDemo:false,
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[11px] font-mono mt-0.5" style={{color:'rgba(255,255,255,0.35)'}}>
                {REPORTS.length} laporan ({demoCount} demo) · Kab. Bandung · {analytics.total.toLocaleString('id')} atlet
              </p>
            </div>
          </div>
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap mb-1">
                        ...
                        {r.isFeatured && (
                          <span className="text-[8px] px-1.5 py-0.5 rounded font-black"
                            style={{background:'rgba(56,189,248,0.15)',color:ACCENT,border:`1px solid ${ACCENT}30`}}>
                            ⭐ FEATURED
                          </span>
                        )}
                        {r.isDemo && (
