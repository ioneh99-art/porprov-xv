...
function ImportTab({ existingAtletCount }: { existingAtletCount: number }) {
  ...
  async function downloadTemplate() {
    ...
    const ws = XLSX.utils.aoa_to_sheet([ALL_TEMPLATE_COLS,
      ['Budi Santoso','3201011234567890','1998-05-14','L','Hockey','3201','Kab. Bogor','M','42','BRI','1234567890'],
      ['Sari Dewi','3201016789012345','2001-11-20','P','Renang','3201','Kab. Bogor','S','37','BCA','0987654321'],
    ])
        ...
        <div>
          ...
          <div className="flex items-center gap-2 flex-wrap text-xs">
            {[
              ...
              {l:'5. Preview & Inject DB',c:'#4ade80'},
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <Card>
          ...
          <div className="p-5 space-y-5">
            ...
            {/* Step 2: upload */}
            <StepRow n={step!=='idle'?'✓':'2'} color={step!=='idle'?'#4ade80':'#60a5fa'}>
              ...
              <>
                ...
                <StepRow n="3" color="#fbbf24">
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-3">
                    {[{l:'✅ OK',v:stats.ok,c:'#4ade80'},{l:'⚠ Warning',v:stats.warn,c:'#fbbf24'},{l:'❌ Error',v:stats.err,c:'#f87171'}]
                      .map(s => (
              ...
              <>
                ...
                <StepRow n="4" color="#4ade80">
                  ...
                  <div className="p-3 rounded-xl mb-3 text-[11px] text-zinc-400 space-y-1"
                    ...
                    <div>• Kontingen: <strong style={{color:ACCENT}}>Kab. Bogor (ID={KONTINGEN_ID})</strong></div>
        ...
        <Card>
          ...
          <div className="p-5 space-y-3">
            {[
              { l:'Template Import Atlet',  d:'File Excel kosong dengan format + instruksi', icon:FileSpreadsheet, c:'#4ade80', badge:'FORMAT RESMI', action: downloadTemplate },
      ...
      {validations.length > 0 && (
        <Card>
          <div className="flex items-center justify-between px-5 py-3.5 border-b" style={{ borderColor:'rgba(255,255,255,0.07)' }}>
            ...
            <div className="flex gap-2">
              {[{l:`✅ ${stats.ok}`,c:'#4ade80'},{l:`⚠ ${stats.warn}`,c:'#fbbf24'},{l:`❌ ${stats.err}`,c:'#f87171'}].map(s => (
          ...
          <div className="overflow-x-auto max-h-[400px] overflow-y-auto" style={{ scrollbarWidth:'thin', scrollbarColor:`${ACCENT}25 transparent` }}>
            <table className="w-full text-left border-collapse min-w-max">
              <thead>
                <tr className="sticky top-0" style={{ background:'rgba(2,13,6,0.98)', borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
              ...
              <tbody>
                {displayed.map(v => {
                  const stc = v.status==='OK'?'#4ade80':v.status==='WARNING'?'#fbbf24':'#f87171'
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={saveAll} disabled={saving || !hasChanges}
            ...
            style={{ background: hasChanges?'rgba(74,222,128,0.15)':'rgba(255,255,255,0.04)', border:`1px solid ${hasChanges?'rgba(74,222,128,0.35)':'rgba(255,255,255,0.1)'}`, color: hasChanges?'#4ade80':'rgba(255,255,255,0.3)' }}>
            {saving ? <><Spinner size={14} color="#4ade80"/> Menyimpan...</> : <><Save size={14}/> Simpan Semua</>}
          </button>
        </div>
      </div>
      ...
      <Card>
        <div className="overflow-x-auto max-h-[600px] overflow-y-auto" style={{ scrollbarWidth:'thin', scrollbarColor:`${ACCENT}25 transparent` }}>
          <table className="w-full border-collapse">
            <thead>
              <tr className="sticky top-0 z-10" style={{ background:'rgba(2,13,6,0.98)', borderBottom:'2px solid rgba(255,255,255,0.08)' }}>
...
function CaborTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Add new form */}
      {addingNew && (
        <div className="rounded-2xl p-4 flex items-center gap-3" style={{ background:'rgba(0,255,170,0.05)', border:`1px solid ${ACCENT}20` }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px" style={{ background:'rgba(255,255,255,0.05)' }}>
          {filtered.map(cabor => (
            <div key={cabor.id} className="p-4 flex items-center gap-3 transition-colors"
              style={{ background: cabor.is_active ? 'rgba(2,13,6,0.7)' : 'rgba(0,0,0,0.5)' }}>
              {/* Active indicator */}
              <div className="w-1.5 h-1.5 rounded-full flex-shrink-0"
                style={{ background: cabor.is_active ? '#4ade80' : '#475569' }}/>
                ...
                {editingId===cabor.id ? (
                  <>
                    <button onClick={()=>saveEdit(cabor.id)} disabled={saving===cabor.id}
                      ...
                      {saving===cabor.id ? <Spinner size={12} color="#4ade80"/> : <Check size={12} className="text-green-400"/>}
                    </button>
                  ...
                  <>
                    ...
                    <button onClick={()=>toggleActive(cabor.id, cabor.is_active)} disabled={saving===cabor.id}
                      ...
                      {saving===cabor.id
                        ? <Spinner size={12} color={cabor.is_active?'#4ade80':'#64748b'}/>
                        : cabor.is_active
...
export default function DataGatewayPage() {
  ...
  return (
    <div className="min-h-screen text-zinc-300 font-sans" style={{ background:'linear-gradient(135deg,#020d06 0%,#030e08 100%)' }}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[11px] font-mono mt-0.5" style={{ color:'rgba(255,255,255,0.35)' }}>
                Manajemen Database Global PORPROV XV · Kab. Bogor
              </p>
            </div>
          </div>
