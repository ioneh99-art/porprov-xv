...
export default function OperatorKejuaraanPage() {
  ...
  const loadData = async () => {
        ...
        // Step 1: ambil dulu atlet yang cabor_id = cabor operator
        ...
        const { data } = await supabase
            ...
            .select(`
            *,
            atlet(nama_lengkap, no_ktp, cabor_id, cabang_olahraga(nama), kontingen(nama))
            `)
  ...
  const pending = kejuaraan.filter(k => k.status === 'Menunggu Cabor').length
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-7">
        <div>
          <h1 className="text-lg font-semibold text-white">Validasi Kejuaraan Atlet</h1>
          <p className="text-slate-500 text-xs mt-0.5">
            Validasi teknis riwayat kejuaraan atlet cabor kamu
          </p>
        </div>
        {pending > 0 && (
          <div className="flex items-center gap-2 bg-blue-500/10 border border-blue-500/20 rounded-xl px-4 py-2">
            ...
            <span className="text-blue-400 text-xs font-medium">
              {pending} menunggu validasi teknis
            </span>
          </div>
        )}
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-800 flex items-center gap-3 flex-wrap">
          ...
          <div className="flex gap-1.5 flex-wrap">
            {['Menunggu Cabor', 'Menunggu Admin', 'Verified', 'Ditolak Cabor', 'Semua'].map(f => (
          ...
          <div className="text-center py-16">
            ...
            <div className="text-slate-600 text-sm">
              {filter === 'Menunggu Cabor'
                ? '✅ Tidak ada yang menunggu validasi'
                : 'Tidak ada data'}
            </div>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <div className="w-5 h-5 rounded-full bg-emerald-500/10 flex items-center justify-center">
                        <span className="text-emerald-400 text-[9px] font-bold">A</span>
                      </div>
                    ...
                    {/* Catatan KONIDA */}
                    {k.catatan_konida && k.catatan_konida !== 'Disetujui oleh KONIDA' && (
                      <div className="mt-2 text-amber-400 text-xs bg-amber-500/5 border border-amber-500/10 px-3 py-2 rounded-lg">
                        📝 KONIDA: {k.catatan_konida}
                      </div>
                    )}

                    {k.status?.includes('Ditolak') && k.catatan_cabor && (
                  ...
                  <div className="flex flex-col gap-2 flex-shrink-0">
                    ...
                    {k.status === 'Menunggu Admin' && (
                      <span className="text-purple-400 text-[10px] text-right">
                        ✓ Diteruskan<br/>ke Admin
                      </span>
                    )}
                    {k.status === 'Verified' && (
