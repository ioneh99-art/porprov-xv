'use client'
// src/app/operator/pentathlon/kejuaraan/page.tsx
// Basis Data Prestasi Atlet Pentathlon — mirror kab.bandung design, amber accent
...
import {
  ...
  RefreshCw, CheckCircle, Filter, ArrowLeft,
} from 'lucide-react'
...
function hitungUmur(tgl: string | null) {
  if (!tgl) return 0
...
export default function PageKejuaraanPentathlon() {
  ...
  const [selectedAtlet, setSelectedAtlet] = useState<AtletRow | null>(null)
  const [filterGender, setFilterGender] = useState<FilterGender>('Semua')
  const [filterType,   setFilterType]   = useState<FilterType>('all')
  const [expandedGroup, setExpandedGroup] = useState<string>('Putra')
    ...
    async function load() {
      try {
        ...
        const atletRes = await sb
          .from('atlet')
          .select('id, nama_lengkap, gender, tgl_lahir, no_registrasi_koni, status_registrasi')
          .eq('cabor_id', caborId)
  ...
  const grouped = useMemo(() => {
    let list = atlets
    if (search) list = list.filter(a => a.nama_lengkap.toLowerCase().includes(search.toLowerCase()))
    if (filterGender !== 'Semua') list = list.filter(a => a.gender === filterGender)
    const putra = list.filter(a => a.gender === 'L')
    const putri = list.filter(a => a.gender === 'P')
    const result: Record<string, AtletRow[]> = {}
    if (putra.length > 0) result['Putra'] = putra
    if (putri.length > 0) result['Putri'] = putri
    if (putra.length === 0 && putri.length === 0 && list.length > 0) result['Atlet'] = list
    return result
  ...
  const kpi = useMemo(() => {
    ...
    return { totalAtlet, putra, putri, totalRecords, totalDemo, totalReal, totalEmas, totalPerak, totalPerunggu, atletDenganRekam }
        ...
        <div className="max-w-[1600px] mx-auto flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-4">
            <Link href="/operator/pentathlon"
              className="p-2 rounded-xl transition hover:bg-amber-500/10"
            ...
            <div>
              ...
              <p className="text-[11px] font-mono mt-0.5" style={{ color: 'rgba(255,255,255,0.35)' }}>
                {kpi.totalRecords} records · {kpi.atletDenganRekam} atlet · {caborNama}
              </p>
            </div>
          </div>
      ...
      <main className="flex-1 p-6 max-w-[1600px] w-full mx-auto relative z-10 space-y-5">
        ...
        {showDemoBanner && kpi.totalDemo > 0 && (
          <div {...ani(0)} className="rounded-2xl p-4 relative overflow-hidden"
            style={{ background: 'linear-gradient(135deg, rgba(245,158,11,0.06) 0%, rgba(217,119,6,0.04) 100%)', border: '1px solid rgba(245,158,11,0.25)' }}>
            ...
            <div className="relative flex items-start gap-3 flex-wrap">
              ...
              <div className="flex-1 min-w-[200px]">
                <div className="text-sm font-black text-amber-300 mb-1">Mode Preview Demo · {kpi.totalDemo} records</div>
                <p className="text-[11px] text-amber-200/80 leading-relaxed">
                  Sebagian data merupakan data demo/preview. Input data real via Data Gateway.
                  {kpi.totalReal > 0 && <> · Sudah ada <strong className="text-amber-400">{kpi.totalReal} data REAL</strong>.</>}
                </p>
              </div>
        ...
        <div {...ani(20)} className="grid grid-cols-4 gap-4">
            ...
            <div className="flex items-start justify-between mb-3">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] text-zinc-500 uppercase tracking-wider">Atlet Pentathlon</span>
                </div>
                <div className="text-5xl font-light text-white">{kpi.totalAtlet}</div>
                <div className="text-[11px] mt-1" style={{ color: `${ACCENT}70` }}>
                  {kpi.atletDenganRekam} atlet punya rekam prestasi
                </div>
              </div>
              <div className="text-right flex-shrink-0">
                <div className="text-[10px] text-zinc-500 mb-2">Gender</div>
                <div className="flex gap-2">
                  <div className="px-2.5 py-1.5 rounded-xl text-center" style={{ background: `${ACCENT}12`, border: `1px solid ${ACCENT}20` }}>
                    <div className="text-lg font-black" style={{ color: ACCENT }}>{kpi.putra}</div>
                    <div className="text-[9px] text-zinc-600">Putra</div>
                  </div>
                  <div className="px-2.5 py-1.5 rounded-xl text-center" style={{ background: 'rgba(244,114,182,0.1)', border: '1px solid rgba(244,114,182,0.2)' }}>
            ...
            <div className="space-y-2.5">
              <div>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] text-sky-400 flex items-center gap-1"><CheckCircle size={9} /> REAL DATA</span>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] text-amber-400 flex items-center gap-1"><AlertTriangle size={9} /> DEMO DATA</span>
          ...
          {/* Medali */}
          <div className="rounded-2xl p-5 relative" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,215,0,0.12)' }}>
            ...
            {kpi.totalEmas + kpi.totalPerak + kpi.totalPerunggu === 0 ? (
              <div>
                ...
                <Link href="/operator/pentathlon/data-gateway"
          ...
          <div className="rounded-2xl overflow-hidden flex flex-col"
            ...
            <div className="px-4 py-3.5 border-b flex items-center justify-between"
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-white">Daftar Atlet</span>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1.5"
              ...
              {Object.entries(grouped).map(([group, list]) => {
                const isExp = expandedGroup === group
                return (
                  <div key={group} className="rounded-xl overflow-hidden"
                    ...
                    <div className="p-3 flex items-center justify-between cursor-pointer"
                      onClick={() => setExpandedGroup(isExp ? '' : group)}>
                      <div>
                        <div className="text-xs font-bold text-zinc-200">{group}</div>
                        <div className="text-[10px] mt-0.5" style={{ color: 'rgba(255,255,255,0.3)' }}>
                          {list.length} atlet
                        </div>
                      </div>
                      {isExp ? <ChevronUp size={13} style={{ color: ACCENT }} /> : <ChevronDown size={13} style={{ color: 'rgba(255,255,255,0.25)' }} />}
                    </div>
                    {isExp && (
                      <div className="border-t px-2 pb-2" style={{ borderColor: `${ACCENT}12` }}>
                        {list.map(a => {
                          const tr    = recordsByAtlet[a.id] || []
                          const emas  = tr.filter(r => r.hasil === 'Emas').length
                          ...
                          return (
                            <div key={a.id}
                              ...
                              style={{ background: isSel ? `${ACCENT}10` : 'transparent', border: `1px solid ${isSel ? `${ACCENT}25` : 'transparent'}` }}
                              ...
                              <div className="flex items-center gap-2.5">
                                <div className="w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0"
                                  style={{ background: emas > 0 ? 'rgba(251,191,36,0.15)' : 'rgba(255,255,255,0.06)', border: `1px solid ${emas > 0 ? 'rgba(251,191,36,0.3)' : 'rgba(255,255,255,0.08)'}` }}>
                                  {emas > 0 ? <Flame size={11} style={{ color: '#fbbf24' }} /> : <User size={11} style={{ color: 'rgba(255,255,255,0.3)' }} />}
                                </div>
                                <div>
                                  <div className="text-xs font-bold flex items-center gap-1.5"
                                    style={{ color: isSel ? ACCENT : emas > 0 ? '#fbbf24' : 'rgba(255,255,255,0.7)' }}>
                                    {a.nama_lengkap}
                                    {hasDemo && <span className="text-[8px] px-1 py-0.5 rounded font-black" style={{ background: 'rgba(251,191,36,0.15)', color: '#fbbf24' }}>DEMO</span>}
                                  </div>
                                  <div className="text-[9px] mt-0.5" style={{ color: 'rgba(255,255,255,0.25)' }}>
                                    {hitungUmur(a.tgl_lahir) > 0 ? `${hitungUmur(a.tgl_lahir)}th · ` : ''}{a.gender === 'L' ? 'Putra' : 'Putri'}
                                    {emas > 0 && <span className="text-amber-500 ml-1">· {emas} Emas</span>}
                                  </div>
                                </div>
                              </div>
              ...
              {Object.keys(grouped).length === 0 && (
                <div className="p-8 text-center text-zinc-600 text-xs">Tidak ada atlet</div>
              )}
            </div>
          </div>
                ...
                <div className="p-6 border-b relative overflow-hidden"
                  ...
                  <div className="flex items-center gap-5">
                    <div className="w-16 h-16 rounded-2xl flex items-center justify-center flex-shrink-0"
                      style={{ background: isElite ? 'rgba(251,191,36,0.1)' : 'rgba(255,255,255,0.05)', border: `2px solid ${isElite ? 'rgba(251,191,36,0.4)' : `${ACCENT}30`}`, boxShadow: isElite ? '0 0 20px rgba(251,191,36,0.15)' : 'none' }}>
                      {isElite ? <Flame size={28} style={{ color: '#fbbf24' }} /> : <User size={28} style={{ color: 'rgba(255,255,255,0.4)' }} />}
                    </div>
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 mt-2 flex-wrap">
                        {[
                          { l: 'Usia', v: hitungUmur(selectedAtlet.tgl_lahir) > 0 ? `${hitungUmur(selectedAtlet.tgl_lahir)} Tahun` : '—', c: 'rgba(255,255,255,0.6)' },
                          { l: 'Gender', v: selectedAtlet.gender === 'L' ? 'Putra' : 'Putri', c: selectedAtlet.gender === 'L' ? ACCENT : '#f472b6' },
                          { l: 'Status', v: selectedAtlet.status_registrasi || '—', c: selectedAtlet.status_registrasi === 'Verified' ? '#22d3ee' : '#6b7280' },
                        ].map(f => (
                ...
                <div className="flex-1 overflow-y-auto p-6"
                  ...
                  <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                    <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-widest flex items-center gap-2">
                      <Calendar size={12} style={{ color: ACCENT }} /> Riwayat Kompetisi
                    </h3>
                  ...
                  {selectedHistory.length === 0 ? (
                    <div className="rounded-2xl p-6 text-center"
                      ...
                      <p className="text-xs text-zinc-600 leading-relaxed mb-3">Riwayat kompetisi atlet ini belum diinput.</p>
                      <Link href="/operator/pentathlon/data-gateway"
                        className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold"
                        style={{ background: `${ACCENT}15`, border: `1px solid ${ACCENT}30`, color: ACCENT }}>
                        + Input Prestasi via Data Gateway
                      </Link>
                    </div>
                  ) : (
                        ...
                        return (
                          <div key={h.id} className="relative pl-6">
                            ...
                            <div className="rounded-xl p-4 relative overflow-hidden"
                              style={{ background: h.is_demo ? 'rgba(251,191,36,0.025)' : 'rgba(255,255,255,0.025)', border: `1px solid ${h.is_demo ? 'rgba(251,191,36,0.2)' : 'rgba(255,255,255,0.07)'}` }}>
                              {h.is_demo && (
                                ...
                                <div className="flex items-start justify-between gap-3 mb-1.5">
                                  <div className="flex-1">
                                    ...
                                    <div className="flex items-center gap-3 text-[10px]" style={{ color: 'rgba(255,255,255,0.4)' }}>
                                      ...
                                      {h.nomor_tanding && <span>{h.nomor_tanding}</span>}
                                    </div>
                                    {h.catatan && (
