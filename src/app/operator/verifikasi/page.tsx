...
export default function OperatorVerifikasiPage() {
  ...
  const handleApprove = async (atletId: number) => {
    ...
    try {
      ...
      await supabase.rpc('approve_cabor', {
        atlet_id: atletId,
        user_id: me.id,
        catatan: 'Disetujui oleh operator cabor'
      })
      await loadData()
    } finally {
      setProcessing(null)
  ...
  const handleReject = async (atletId: number) => {
    ...
    try {
      ...
      await supabase.rpc('reject_cabor', {
        atlet_id: atletId,
        user_id: me.id,
        catatan
      })
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-800">
                {['Nama', 'Kontingen', 'Gender', 'Status', 'Catatan', 'Aksi'].map(h => (
                ...
                <tr key={a.id} className="border-b border-slate-800/40 hover:bg-slate-800/20 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    ...
                    {a.status_registrasi === 'Menunggu Admin' && (
                      <span className="text-purple-400 text-[10px]">✓ Diteruskan ke Admin</span>
                    )}
