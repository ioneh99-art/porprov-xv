// Komponen modal export — taruh di dalam AtletKabBogor page
// Butuh: npm install xlsx

import { useState, useMemo } from 'react'
import { Download, X, FileSpreadsheet, CheckCircle, Clock, XCircle, FileCheck, Users, Filter } from 'lucide-react'
...
export function ExportModal({ data, onClose }: Props) {
  ...
  const previewData = useMemo(()=>{
    ...
    if (statusFilter !== 'semua') {
      filtered = filtered.filter(a => a.status_registrasi === statusFilter)
    }
    if (caborFilter !== 'semua') {
      filtered = filtered.filter(a => (a.cabor_nama_raw||'Belum Ditentukan') === caborFilter)
    }
    return filtered
  },[data, statusFilter, caborFilter])

  // Hitung per cabor untuk preview
  ...
  async function handleExport() {
    ...
    try {
      ...
      const rekapRows = perCabor.map(([cabor, total], i) => {
        const atletCabor = previewData.filter(a=>(a.cabor_nama_raw||'Belum Ditentukan')===cabor)
        return {
          'No':        i+1,
          'Cabor':     cabor,
          'Total':     total,
          'Putra':     atletCabor.filter(a=>a.gender==='L').length,
          'Putri':     atletCabor.filter(a=>a.gender==='P').length,
          'Lokal':     atletCabor.filter(a=>a.kode_asal_daerah?.startsWith('3201')).length,
          'Non-Lokal': atletCabor.filter(a=>!a.kode_asal_daerah?.startsWith('3201')).length,
        }
      })

      const ws2 = XLSX.utils.json_to_sheet(rekapRows)
      ws2['!cols'] = [{wch:5},{wch:30},{wch:8},{wch:8},{wch:8},{wch:8},{wch:10}]

      // ── Sheet 3: Summary ─────────────────────────────
      const summaryRows = [
        { 'Keterangan':'Kontingen',          'Nilai':'Kabupaten Bogor'              },
        ...
        { 'Keterangan':'Atlet Lokal',        'Nilai':previewData.filter(a=>a.kode_asal_daerah?.startsWith('3201')).length },
        { 'Keterangan':'Atlet Non-Lokal',    'Nilai':previewData.filter(a=>!a.kode_asal_daerah?.startsWith('3201')).length },
      ]
      ...
      const filename   = `AtletKabBogor_${statusStr}_${caborStr}_${dateStr}.xlsx`
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-[580px] rounded-2xl overflow-hidden"
        style={{background:'#040f08',border:'1px solid rgba(0,255,170,0.2)',boxShadow:'0 25px 60px rgba(0,0,0,0.8)'}}>

        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b"
          style={{borderColor:'rgba(0,255,170,0.1)',background:'rgba(0,255,170,0.04)'}}>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center"
              style={{background:'rgba(0,255,170,0.12)',border:'1px solid rgba(0,255,170,0.25)'}}>
              <FileSpreadsheet size={20} style={{color:ACCENT}}/>
            </div>
            <div>
              <div className="text-white font-bold text-base">Export Data Atlet</div>
              <div className="text-[11px] mt-0.5" style={{color:'rgba(255,255,255,0.35)'}}>Format Excel (.xlsx) · 3 sheet</div>
            </div>
          </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            ...
            <select value={caborFilter} onChange={e=>setCaborFilter(e.target.value)}
              ...
              {caborList.map(c=>(
                <option key={c} value={c} style={{background:'#040f08'}}>
              ...
              <>
                ...
                <div className="flex gap-3 mb-3 flex-wrap">
                  {[
                    ...
                    {l:'Lokal',   v:previewData.filter(a=>a.kode_asal_daerah?.startsWith('3201')).length, c:ACCENT},
                    {l:'Non-Lokal',v:previewData.filter(a=>!a.kode_asal_daerah?.startsWith('3201')).length,c:'#fb923c'},
                  ].map(s=>(
                      ...
                      <div key={cabor} className="flex items-center gap-3 text-xs">
                        ...
                        <div className="flex-1 h-1 rounded-full overflow-hidden" style={{background:'rgba(255,255,255,0.06)'}}>
                          <div className="h-full rounded-full"
                            style={{width:`${Math.max(total/previewData[0]?.id||1*0.01,0.05)*100}%`,background:ACCENT}}/>
                        </div>
                ...
                <div className="mt-3 pt-3 border-t flex gap-2 flex-wrap" style={{borderColor:'rgba(255,255,255,0.06)'}}>
                  {[
                    {l:'Sheet 1: Data Atlet',   sub:`${previewData.length} baris · 16 kolom`},
                    {l:'Sheet 2: Rekap Cabor',  sub:`${perCabor.length} cabor`},
                    {l:'Sheet 3: Summary',      sub:'Ringkasan export'},
                  ].map(s=>(
        ...
        <div className="px-6 pb-6 flex gap-3">
          ...
          <button onClick={handleExport}
            ...
            style={{background:done?'rgba(74,222,128,0.15)':`rgba(0,255,170,0.15)`,border:`1px solid ${done?'rgba(74,222,128,0.4)':`rgba(0,255,170,0.35)`}`,color:done?'#4ade80':ACCENT}}>
            {done
