...
import { LayoutDashboard, Users, BarChart2, LogOut, ChevronRight } from 'lucide-react'
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-56 bg-slate-900 border-r border-slate-800 flex flex-col z-20">
      <div className="px-5 py-5 border-b border-slate-800">
        ...
        <span className="inline-block mt-2 bg-amber-500/20 text-amber-400 text-[9px] px-2 py-0.5 rounded font-semibold tracking-wider border border-amber-500/20">
          KONIDA
        </span>
      </div>
          ...
          return (
            <Link key={href} href={href}
              className={`flex items-center gap-2.5 px-2.5 py-2 rounded-lg mb-0.5 text-sm transition-all
                ${active ? 'bg-amber-500/10 text-amber-400' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`}>
              <Icon size={16} />
